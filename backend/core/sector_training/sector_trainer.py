...
from backend.core.sector_training.sector_dataset_builder import default_regime_profile_for_horizon, apply_regime_filter, build_sector_row_mask
...
def _stream_sector_validation_sample(parquet_path: Path, feature_cols: List[str], target_col: str, sector_mask_fn, *,
                                    max_rows: int, batch_size: int, y_clip_low: float, y_clip_high: float) -> Tuple[np.ndarray, np.ndarray]:
...
def train_sector_models(
    ...
    **kwargs: Any,                       # ✅ swallow any future args safely
    ...
    try:
        ...
        ) if "symbol" not in X_latest.columns else infer_sector_labels_from_features(X_latest.reset_index())
    ...
    if sector_labels is not None:
        sectors = sorted(set([str(x).upper().strip() for x in sector_labels.values if x]))
    if not sectors:
        sectors = ["UNKNOWN"]

    max_workers = env_max_workers(8)
    ...
    def train_one_sector(sec: str) -> Tuple[str, Dict[str, Any]]:
        ...
        sector_summary: Dict[str, Any] = {"status": "ok", "horizons": {}}

        # >>> ADDED: prune feature list for this sector (non-destructive)
        pr = prune_features_for_sector(sec_norm, feature_cols)
        feature_cols_for_sector = pr.kept
        # <<< END ADDITION
        for h in core_training.HORIZONS:
            tgt_ret = f"target_ret_{h}"
            if tgt_ret not in target_cols:
                continue

            if sector_labels is not None:
                df_sector = pd.DataFrame({'symbol': sector_labels.index.astype(str), 'sector': sector_labels.values})
                df_sector = df_sector[df_sector['sector'].astype(str).str.upper().str.strip() == sec_norm]
                symbols = set(df_sector['symbol'].astype(str).str.upper().unique())
            else:
                symbols = set()
