...
def _run_late_start_bots():
    """Worker thread for late startup catch-up runs."""
    try:
        ...
        for func, tag in [(run_1w, "1w"), (run_2w, "2w"), (run_4w, "4w")]:
            try:
                func("full")
                print(f"✅ {tag} bots complete (startup catch-up)")
            except Exception as e:
                print(f"⚠️ Auto-start {tag} bots failed: {e}")
    except Exception as e:
        print(f"⚠️ Late-start bot runner failed to import or execute: {e}")


def auto_start_bots_on_late_launch():
    """If system starts after 7:30 ET but before close, run full bot cycle once."""
    ...
    if start_cutoff <= now.time() <= close_cutoff and now.weekday() < 5:
        t = threading.Thread(target=_run_late_start_bots, name="late-start-bots", daemon=True)
        t.start()
    else:
        print("⏳ Startup outside market hours — skipping auto bot start.")

# Run once on backend startup
threading.Thread(target=auto_start_bots_on_late_launch, daemon=True).start()
