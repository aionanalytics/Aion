# dt_backend/ai_model_intraday.py — v1.0
# Loads intraday model + features, scores latest snapshot per symbol,
# writes compact signals JSON to: ml_data_dt/signals/intraday_predictions.json
...
def _latest_snapshot(df: pd.DataFrame) -> pd.DataFrame:
    # Take the last row per symbol as the "now" snapshot
...
def score_intraday_tickers() -> Dict[str, Any]:
    ...
    # Coerce types and fill
    for c in X.columns:
        X[c] = pd.to_numeric(X[c], errors="coerce")
    X = X.fillna(0)

    # LightGBM Booster requires numpy
    pred_proba = booster.predict(X.values)  # shape [n, n_classes]
    ...
    # Save compact signals map
    try:
        with open(OUT_PATH, "w", encoding="utf-8") as f:
            json.dump({"generated_at": datetime.utcnow().isoformat(), "rows": list(results.values())}, f, indent=2)
