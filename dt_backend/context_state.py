# backend/context_state.py — v1.1
"""
Builds per-ticker and global context features that mimic human awareness.
Inputs (best-effort, all optional):
  • news_intel outputs (stance, buzz, event impulses)
  • macro_fetcher features (breadth, vol proxy)
  • rolling cache (trend flags, sector momentum if present)
  • optional social_sentiment_YYYYMMDD.json (Reddit/FinBERT crowd data)
Outputs:
  • Writes context.* dict on each symbol in Rolling
  • Writes ml_data/market_state.json (global market regime snapshot)
Safe: runs with missing files; uses neutral defaults.
"""
...
def _mk_global_state(macro_js: Dict[str, Any] | None) -> Dict[str, Any]:
    # Derive risk-on/off + macro_vol from macro features (breadth_pos, vol_z, etc.)
    breadth = 0.0
    vol_z   = 0.0
    ...
    state = "risk_on"
    if macro_vol > 0.65 or breadth < -10:  # crude but effective defaults
        state = "risk_off"
...
def update() -> Dict[str, Any]:
    ...
    # Load latest macro + news summaries (best-effort)
    ...
    # Ticker-level news aggregates (base)
    tnews = _ticker_news_map(news_js)

    # ------------------------------------------------------------------
    # NEW: Optional merge of social sentiment (Reddit/FinBERT crowd data)
    # ------------------------------------------------------------------
    ...
    if isinstance(social_js, dict):
        ...
        for sym, node in (data_block or {}).items():
            ...
            if sym not in tnews:
                tnews[sym] = {}
            # Blend sentiment 50/50 if both exist
    ...
    log(f"[context_state] ✅ context updated for {len(rolling):,} symbols; global={glbl}")
